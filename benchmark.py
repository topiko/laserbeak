...
# from laserbeakdeformdfnet import DFNet
...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir", default="./data", type=str, help="Set root data directory."
    )
    parser.add_argument(
        "--ckpt_dir",
        default="./checkpoint",
    ...
    parser.add_argument(
        "--exp_name", type=str, default=f'{time.strftime("%Y%m%d-%H%M%S")}', help=""
    )
    ...
    parser.add_argument(
        "--lr", default=2e-3, type=float, help="Set optimizer learning rate."
    )
    ...
    for i in range(len(thresholds)):
        ...
        for j in range(len(y_pred)):
            ...
            # below confidence threshold
                ...
                res[i][1] += 1  # TN
    if print_res:
        print("Thr:\tTrue Pos.\tTrue Neg.\tFalse Pos.\tFalse Neg.\tPre.\tRec.")
        for i, th in enumerate(thresholds):
            ...
            print(
                f"{th:.3f}:\t{int(res[i][0])}\t{int(res[i][1])}\t{int(res[i][2])}\t{int(res[i][3])}\t{pre:.3f}\t{rec:.3f}"
            )
    return res
...
if __name__ == "__main__":
    ...
    opt_lr = args.lr
    opt_betas = (0.9, 0.999)
    opt_wd = 0.001 if not args.orig_optim else 0.0
    ...
    trainloader, valloader, testloader, classes = load_data(
        ...
        root=root,
        val_perc=0.1 if use_val else 0.0,
    ...
    if args.orig_optim:
        optimizer = optim.Adamax(
            params, lr=opt_lr, betas=opt_betas, weight_decay=opt_wd
        )
    else:
        optimizer = optim.AdamW(params, lr=opt_lr, betas=opt_betas, weight_decay=opt_wd)
    ...
    def epoch_iter(dataloader, eval_only=False, desc="", return_raw=False):
        ...
        with tqdm(
            ...
            for batch_idx, (inputs, targets, sample_sizes) in enumerate(pbar):
                ...
                if return_raw:
                    all_y_prob = np.concatenate((all_y_prob, y_prob.numpy(force=True)))
                    all_y_pred = np.concatenate((all_y_pred, y_pred.numpy(force=True)))
                    all_targets = np.concatenate(
                        (all_targets, targets.numpy(force=True))
                    )
        ...
        if return_raw:
            return epoch_loss, epoch_acc, (all_y_prob, all_y_pred, all_targets)
        else:
    ...
    if eval_only:
        if resumed:
            ...
            if include_unm:
                ...
                thresholds = np.linspace(0.0, 0.9, num=10, endpoint=False)
                thresholds = np.concatenate(
                    (thresholds, np.linspace(0.9, 1.0, num=10, endpoint=False))
                )

            # train eval
            train_loss, train_acc, raw_res = epoch_iter(
                trainloader, desc="Train", eval_only=True, return_raw=True
            )
            if include_unm:
                res = calc_ow(
                    raw_res[0],
            ...
            if valloader is not None:
                val_loss, val_acc, raw_res = epoch_iter(
                    valloader, desc="Val.", eval_only=True, return_raw=True
                )
                if include_unm:
                    res = calc_ow(
                        raw_res[0],
            ...
            if testloader is not None:
                test_loss, test_acc, raw_res = epoch_iter(
                    testloader, desc="Test", eval_only=True, return_raw=True
                )
                if include_unm:
                    res = calc_ow(
                        raw_res[0],
        ...
        try:
            for epoch in range(last_epoch + 1, epochs):
                ...
                train_loss, train_acc = epoch_iter(
                    trainloader, desc=f"Epoch {epoch} Train"
                )
                ...
                if valloader is not None:
                    ...
                    with torch.no_grad():
                        val_loss, val_acc = epoch_iter(
                            valloader, eval_only=True, desc=f"Epoch {epoch} Val."
                        )
                    metrics.update({"val_loss": val_loss, "val_acc": val_acc})

                if save_best_epoch:
                    best_val_loss = min(
                        [999] + [metrics["val_loss"] for metrics in history.values()]
                    )
                    if metrics["val_loss"] < best_val_loss:
                        checkpoint_path_epoch = (
                            f"{checkpoint_dir}/{checkpoint_fname}/best.pth"
                        )
                ...
                if (epoch % ckpt_period) == (ckpt_period - 1):
                    ...
                    checkpoint_path_epoch = (
                        f"{checkpoint_dir}/{checkpoint_fname}/e{epoch}.pth"
                    )
                    print(
                        f"Saving end-of-cycle checkpoint to {checkpoint_path_epoch}..."
                    )
                    torch.save(
                        {
                            "epoch": epoch,
            ...
            if testloader is not None:
                ...
                net.eval()
                with torch.no_grad():
                    test_loss, test_acc = epoch_iter(
                        testloader, eval_only=True, desc=f"Epoch {epoch} Test"
                    )
